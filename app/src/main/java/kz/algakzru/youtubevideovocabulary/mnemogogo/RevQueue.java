...
class RevQueue {
    ...
    RevQueue(int size, long days, Config c, Progress p, int days_left) {
        ...
        size += new_at_once;
        Log.d("myLogs", "days_left = " + String.valueOf(days_left));
    ...
    private void updateFutureSchedule(Card card, int delta)
    {
            ...
            && (0 <= next_rep) && (next_rep < futureSchedule.length))
        {
            ...
            //Log.d("myLogs", "futureSchedule["+next_rep+"] += "+delta);
        } //else Log.d("myLogs", "futureSchedule != null next_rep = "+next_rep);
    }
        ...
        for (int i=0; i < cards.length; ++i) {
            if (!cards[i].isSkip()) {
                        ...
                        && cards[i].qualifiesForLearnAhead(days_since_start)))
                {
                    ...
                    Log.d("myLogs", "isDueForRetentionRep[" + num_scheduled + "]: " + cards[i].toString());
    
                } else if (cards[i].isDueForAcquisitionRep()) {
                    Log.d("myLogs", "isDueForAcquisitionRep[" + idx_new + "]: " + cards[i].toString());
                    q[idx_new--] = cards[i];
                }
            }
