...
class CardData
{
    ...
    public CardData(Progress p, CardDataSet carddb)
            throws IOException
    {
        progress = p;
        load(carddb);
    ...
    private void load(CardDataSet carddb)
            throws IOException
    {
        int num_cards = 10;
        int overlay;
        ...
        if (cur.getCount() > 0) {
            int i=0;
            while (cur.moveToNext() && (i < num_cards)) {
                overlay = 1;
                if (carddb.cardDataNeeded(i)) {
                    qcard = cur.getString(cur.getColumnIndex("translation"));
                    acard = cur.getString(cur.getColumnIndex("word"));
                    carddb.setCardData(i, qcard, acard, overlay != 0);
                }
                if (i % 100 == 0) {
                    progress.updateOperation(100);
